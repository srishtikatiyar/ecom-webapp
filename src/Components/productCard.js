...
export function ProductCard({src,name,colour,rating,price,brand}){
       ...
       <div><u>specification-</u>
           <div className="innerBlock"><p><u>Colour : </u>   {colour}</p> <p> <u>Brand:</u>  {brand}</p></div></div> 
       <div><u>Rating (Stars):</u>  {rating}</div>
       <div><u>Price:</u>  {price} rupees</div>
       </div>
       <img src={imgcart} className='cartImg' onClick={()=>{console.log("button clicked");
